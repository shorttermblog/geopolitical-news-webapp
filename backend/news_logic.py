...
def fetch_google_news_rss(url: str, query_label: str = "", max_attempts: int = 1):
    """
    Google News RSS fetch.

    max_attempts is set to 1 by default because if Google returns its
    'Sorry...' 503 block page, retries usually do not help and may worsen
    throttling.
    """
    ...
    for attempt in range(max_attempts):
        try:
            if attempt > 0:
                sleep_seconds = attempt * 3 + random.uniform(1.0, 2.5)
            ...
            first_chars = response.text[:120].replace("\n", " ")
