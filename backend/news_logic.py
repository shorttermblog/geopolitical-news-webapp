...
def normalize_queries(queries):
    ...
    Accepts either:
    - list[str]
    - multiline string from a frontend textarea
    """
...
def fetch_google_news_rss(url: str, query_label: str = "", max_attempts: int = 4):
    """
    Fetches Google News RSS using requests instead of letting feedparser
    open the URL directly.

    This is more reliable on Render/cloud hosts because:
    - it sends browser-like headers
    - it retries temporary 429/5xx errors
    - it parses response.content after the HTTP request succeeds
    """
    ...
    for attempt in range(max_attempts):
        try:
            if attempt > 0:
                sleep_seconds = attempt * 2 + random.uniform(0.5, 1.5)
...
def run_monitor(
    ...
    print("[DEBUG] recent_articles:", len(df))
