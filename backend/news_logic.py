...
def get_openai_client():api_key = os.getenv("OPENAI_API_KEY")if not api_key:raise ValueError("Set OPENAI_API_KEY in environment variables.")return OpenAI(api_key=api_key)

def chat_text(system_prompt, user_prompt, model, temperature=0.4):"""Use Chat Completions because the Render OpenAI package may not support Responses."""client = get_openai_client()response = client.chat.completions.create(model=model,messages=[{"role": "system", "content": system_prompt},{"role": "user", "content": user_prompt},],temperature=temperature,)return response.choices[0].message.content or ""

KEYWORD_SUGGESTION_SYSTEM_PROMPT = """You are a professional geopolitical news monitoring assistant.
...
The user may also provide an optional monitoring instruction.This instruction describes the angle, lens, or type of developments the user cares about.Use it to guide query selection when provided.If no monitoring instruction is provided, generate strong general-purpose queries based only on the topic.

The goal is to retrieve recent, relevant, high-signal news articles.

Important:Google News RSS works best with short keyword queries.Do not make queries too specific.Do not combine too many angles in one query.The app will rank and filter articles later, so queries should be broad enough to retrieve useful results.

First, infer the most likely geopolitical category internally.Do not output the category unless explicitly requested.
...
War / armed conflict guidance:For wars and armed conflicts, cover a balanced mix of:
...
Diplomacy / negotiations guidance:For diplomatic topics, use angles such as:
...
Sanctions / economic pressure guidance:For sanctions and economic-pressure topics, use angles such as:
...
Middle East guidance:For Middle East conflicts or tensions, use angles such as:
...
Russia / Ukraine guidance:For Russia-Ukraine topics, use angles such as:
...
China / Taiwan / Asia-Pacific guidance:For China, Taiwan, or Asia-Pacific tensions, use angles such as:
...
Energy and shipping risk guidance:For geopolitical topics with energy or trade implications, use angles such as:
...
Humanitarian impact guidance:For humanitarian crises, use angles such as:
...
International institutions guidance:For UN, NATO, EU, G7, or other institutions, use angles such as:
...
Return queries in this exact shape:{"queries": ["query one","query two"]}"""

def extract_json_object(text):text = str(text or "").strip()
...
def suggest_news_queries(topic: str, user_prompt: str = "", n: int = 5):if not topic or not topic.strip():raise ValueError("Topic cannot be empty.")
...
Optional user monitoring instruction:{monitoring_instruction}
...
Good examples for topic "Iran war":Iran conflictIran Israel escalationUS Iran talksIran sanctionsHormuz oil risk

Good examples for topic "Ukraine war":Ukraine warRussia Ukraine front lineRussia missile strikesUkraine peace talksNATO Ukraine aid

Good examples for topic "Gaza":Gaza ceasefireIsrael Gaza warGaza humanitarian aidhostage negotiationsUN Gaza crisis

Good examples for topic "Red Sea":Red Sea shippingHouthis attacksRed Sea oil riskUS strikes Yemenshipping route disruption

Bad examples:Iran Israel US sanctions ceasefire diplomacy oil riskUkraine Russia NATO weapons aid front line peace talksMiddle East conflict diplomacy sanctions refugees oil shipping crisisIran war 2024Ukraine war latest 2023

Return JSON in this exact shape:{{"queries": ["query one", "query two"]}}"""
...
def get_embedding_model():global _embedding_model
...
def fetch_google_news(topic: str, max_articles: int = 50):params = urlencode({"q": topic, "hl": "en-US", "gl": "US", "ceid": "US"})url = f"https://news.google.com/rss/search?{params}"
...
def fetch_multiple_queries(queries, max_articles_per_query=50):dfs = []query_counts = {}total_raw_downloaded = 0
...
def fetch_article_body(url, max_chars=3000, timeout=10):if not url or not isinstance(url, str):return ""
...
def enrich_top_articles_with_body(df, top_k=5, max_chars=3000):if df.empty:return df
...
def parse_date(date_str):try:dt = parsedate_to_datetime(date_str)
...
def filter_recent_articles(df, max_age_hours):if df.empty:return df
...
def _keyword_score(text, topic):text_l = str(text or "").lower()
...
def rank_articles(df, topic, recency_window_hours=72, ranking_mode="keyword"):if df.empty:return df
...
def summarize(df, topic, n=5):if df.empty:return {"bullets": [],"takeaway": "No recent articles found for this time filter.",}
...
Article rank: {r.get("rank", "")}Title: {r.get("title", "")}Source: {r.get("source", "")}Date: {r.get("published", "")}Query: {r.get("query", "")}{body_section}"""
...
{{"bullets": [{{"text": "A polished, analytical briefing bullet.","article_refs": [1, 3]}}],"takeaway": "A clear, professional synthesis of the overall geopolitical situation, direction, or risk picture."}}
...
def run_monitor(topic, queries, max_articles=50, top_n=5, max_age_hours=24, ranking_mode="keyword"):active_queries = [str(q).strip() for q in queries if str(q).strip()]
